...
const andhraPradeshBanner = new URL(
  "../assets/states/Andhra Pradesh.jpg",
  import.meta.url
).href;

const modules = import.meta.glob(
  "../assets/andhrapradesh/**/*.{jpg,jpeg,png,webp}",
  {
    eager: true,
    import: "default",
  }
...
DISTRICT_NAMES.forEach((title) => {
  const slug = createSlug(title);

  districtMap[slug] = {
    title,
    slug,
    banner: andhraPradeshBanner,
    sights: [],
  };
});
...
Object.entries(modules).forEach(([path, src]) => {
  ...
  const slug = createSlug(fileName.replace(/\.[^.]+$/, ""));
...
Object.entries(modules).forEach(([path, src]) => {
  ...
  const splitPath = normalizedPath.split("/andhrapradesh/")[1];
  if (!splitPath) return;

  const parts = splitPath.split("/");
  const folderSlug = createSlug(parts[0]);
...
export const AndhraPradesh: React.FC = () => {
  return (
    <div className="min-h-screen bg-white pt-20">
      <div className="relative h-[360px] w-full overflow-hidden">
        <img
          src={andhraPradeshBanner}
          alt="Andhra Pradesh"
        ...
        <div className="relative z-10 flex h-full flex-col items-center justify-center px-6 text-center text-white">
          ...
          <p className="mt-5 max-w-3xl text-lg md:text-xl">
            Explore temples, beaches, caves, valleys and Godavari river views
          </p>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-[1200px] px-6 py-20">
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {DISTRICTS.map((district) => (
            <Link
              key={district.slug}
              to={`/andhrapradesh/${district.slug}`}
              className="group"
            >
...
export const AndhraPradeshDistrictPage: React.FC = () => {
      ...
      <div className="mx-auto max-w-[1200px] px-6 py-16">
        ...
        {district.sights.length === 0 ? (
          <p className="text-gray-500">
            Add images to{" "}
            <span className="font-medium">{`src/assets/andhrapradesh/${district.slug}`}</span>{" "}
