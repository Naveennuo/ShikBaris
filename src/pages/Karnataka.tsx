...
const karnatakaBanner = new URL(
  "../assets/states/Karnataka.jpg",
  import.meta.url
).href;

const modules = import.meta.glob(
  "../assets/karnataka/**/*.{jpg,jpeg,png,webp}",
  {
    eager: true,
    import: "default",
  }
...
Object.entries(modules).forEach(([path, src]) => {
  ...
  const splitPath = normalizedPath.split("/karnataka/")[1];
  if (!splitPath) return;
...
export const Karnataka: React.FC = () => {
  return (
      ...
      <div className="relative h-[360px] w-full overflow-hidden">
        ...
        <div className="relative z-10 flex h-full flex-col items-center justify-center px-6 text-center text-white">
          ...
          <p className="mt-5 max-w-3xl text-lg md:text-xl">
            Explore hills, heritage cities, beaches, forests and temple towns
          </p>
        </div>
      </div>
...
export const KarnatakaDistrictPage: React.FC = () => {
      ...
      <div className="mx-auto max-w-[1200px] px-6 py-16">
        ...
        {district.sights.length === 0 ? (
          <p className="text-gray-500">
            Add images to{" "}
            <span className="font-medium">{`src/assets/karnataka/${district.slug}`}</span>{" "}
