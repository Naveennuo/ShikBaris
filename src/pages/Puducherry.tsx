...
const puducherryBanner = new URL(
  "../assets/states/Puducherry.jpg",
  import.meta.url
).href;

const modules = import.meta.glob(
  "../assets/puducherry/**/*.{jpg,jpeg,png,webp}",
  {
    eager: true,
    import: "default",
  }
...
Object.entries(modules).forEach(([path, src]) => {
  ...
  const splitPath = normalizedPath.split("/puducherry/")[1];
  if (!splitPath) return;
...
export const Puducherry: React.FC = () => {
  return (
    <div className="min-h-screen bg-white pt-20">
      <div className="relative h-[360px] w-full overflow-hidden">
        <img
          src={puducherryBanner}
          alt="Puducherry"
        ...
        <div className="relative z-10 flex h-full flex-col items-center justify-center px-6 text-center text-white">
          ...
          <p className="mt-5 max-w-3xl text-lg md:text-xl">
            Explore beaches, heritage quarters, riverfronts and spiritual landmarks
          </p>
        </div>
      </div>
...
export const PuducherryDistrictPage: React.FC = () => {
      ...
      <div className="mx-auto max-w-[1200px] px-6 py-16">
        ...
        {district.sights.length === 0 ? (
          <p className="text-gray-500">
            Add images to{" "}
            <span className="font-medium">{`src/assets/puducherry/${district.slug}`}</span>{" "}
